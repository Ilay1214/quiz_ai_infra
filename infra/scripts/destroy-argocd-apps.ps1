...
kubectl delete -f "$baseDir\external-secrets-operator.yaml"
Start-Sleep -Seconds 5

Write-Host "Applying aws-load-balancer-controller (Wave 2)..."
kubectl delete -f "$baseDir\aws-load-balancer-controller.yaml"
Start-Sleep -Seconds 5

Write-Host "Applying ingress-nginx (Wave 3)..."
kubectl delete -f "$baseDir\ingress-nginx.yaml"
Start-Sleep -Seconds 5

Write-Host "Applying edge-ingress (Wave 4)..."
kubectl delete -f "$baseDir\edge-ingress.yaml"
Start-Sleep -Seconds 5

Write-Host "Applying external-secrets-config (Wave 5)..."
kubectl delete -f "$baseDir\external-secrets-config.yaml"
Start-Sleep -Seconds 5

Write-Host "Applying quiz-ai-prod (Wave 6)..."
kubectl delete -f "$baseDir\quiz-ai-prod.yaml"
