resource "kubernetes_manifest" "cluster_secret_store" { 
  ...
  region = var.region 
  auth = { jwt = { serviceAccountRef = { name = var.sa_name 
  ...
  resource "kubernetes_namespace_v1" "ns" { 
    ...
    resource "kubernetes_manifest" "app_env" { 
      ...
      depends_on = [ kubernetes_namespace_v1.ns, kubernetes_manifest.cluster_secret_store, time_sleep.wait_for_crds ] 
      } 
      resource "kubernetes_manifest" "mysql_ca" { 
        for_each = (var.enable_k8s && var.create_crd_resources) ? var.namespaces : {} 
        manifest = { apiVersion = "external-secrets.io/v1beta1" 
        kind = "ExternalSecret" 
        metadata = { name = "mysql-ca" 
        namespace = each.value.name } 
        spec = { secretStoreRef = { name = var.secret_store_name 
        kind = "ClusterSecretStore" } 
        refreshInterval = "1h" 
        target = { name = "mysql-ca" 
        creationPolicy = "Owner" 
        template = { type = "Opaque" } } 
        data = [{ secretKey = "ca.pem" 
        remoteRef = { key = each.value.remote_key 
